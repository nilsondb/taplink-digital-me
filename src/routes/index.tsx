...
import { ArrowRight, Sparkles, QrCode, Smartphone, BarChart3, Lock } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Crie sua página personalizada com URL, QR Code e Tags NFC. Dashboard com estatísticas e links ilimitados." },
    ],
  }),
  component: Home,
});
      ...
      <main className="px-6 pt-12 pb-24 max-w-6xl mx-auto">
        <section className="text-center max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 glass px-4 py-2 rounded-full text-xs text-muted-foreground mb-8">
            ...
            Multi-usuário · QR Code · Pronto para NFC
          </div>
          <h1 className="font-display font-bold text-5xl md:text-7xl leading-[1.05] tracking-tight">
            Um link para tudo<br /><span className="gradient-text">que você é</span>
          </h1>
          <p className="mt-6 text-lg text-muted-foreground max-w-xl mx-auto">
            Centralize redes sociais, sites e contatos em uma página premium. Compartilhe por URL, QR Code ou Tag NFC.
          </p>
          <div className="mt-10 flex flex-col sm:flex-row gap-3 justify-center">
            <Link to={user ? "/dashboard" : "/signup"} className="btn-primary inline-flex items-center justify-center gap-2 px-7 py-4 rounded-2xl font-semibold">
              {user ? "Ir para o Dashboard" : "Criar conta grátis"} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </section>
        ...
        <section className="mt-28 grid md:grid-cols-3 gap-4">
          {[
            { icon: Sparkles, title: "Visual premium", desc: "Design escuro, glassmorphism e mobile-first." },
            { icon: BarChart3, title: "Estatísticas", desc: "Acompanhe visualizações e cliques em tempo real." },
            { icon: Lock, title: "Slug personalizado", desc: "Escolha seu link: taplinknfc.com/voce" },
            { icon: QrCode, title: "QR Code embutido", desc: "Compartilhe um QR Code estiloso do seu link." },
            { icon: Smartphone, title: "Pronto para NFC", desc: "Use sua URL em qualquer Tag NFC compatível." },
            { icon: Sparkles, title: "Edição segura", desc: "Apenas você edita seu perfil, com autenticação." },
          ].map((f) => (
        ...
        <section className="mt-28 glass rounded-3xl p-10 text-center">
          <h2 className="text-3xl font-bold">Pronto para criar seu TapLink?</h2>
          <p className="text-muted-foreground mt-3 max-w-lg mx-auto">
            Cadastre-se em segundos. Personalize. Compartilhe.
          </p>
